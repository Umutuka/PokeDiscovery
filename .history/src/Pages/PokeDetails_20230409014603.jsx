...
const Details = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([getPokemon(), getPokemonStatus()]);
    };
    fetchData();
  }, []);
  ...
  const weaknesses = pokemon.types.reduce((accumulator, current) => {
    const { damage_relations } = current.type;
    damage_relations.double_damage_from.forEach((weakness) => {
      if (!accumulator.includes(weakness.name)) {
        accumulator.push(weakness.name);
      }
    });
    return accumulator;
  }, []);

  const strengths = pokemon.types.reduce((accumulator, current) => {
    const { damage_relations } = current.type;
    damage_relations.double_damage_to.forEach((strength) => {
      if (!accumulator.includes(strength.name)) {
        accumulator.push(strength.name);
      }
    });
    return accumulator;
  }, []);

...
import PokemonDefense from "../DetailCells/PokemonDefence";

const Details = () => {
  const { id } = useParams();
  const [pokemon, setPokemon] = useState([]);
  const [description, setSpecies] = useState([]);
  const [typeDefense, setTypeDefense] = useState([]);
  ...
  const getPokemonType = async () => {
    ...
    const typeDefensePromises = types.map(async (type) => {
      const typeRes = await fetch(type.url);
      const typeData = await typeRes.json();
      const damageRelations = typeData.damage_relations;
      const typeObj = {
        name: type.name,
        double_damage_from: damageRelations.double_damage_from.map((damage) => damage.name),
        double_damage_to: damageRelations.double_damage_to.map((damage) => damage.name),
        half_damage_from: damageRelations.half_damage_from.map((damage) => damage.name),
        half_damage_to: damageRelations.half_damage_to.map((damage) => damage.name),
        no_damage_from: damageRelations.no_damage_from.map((damage) => damage.name),
        no_damage_to: damageRelations.no_damage_to.map((damage) => damage.name),
      };
      return typeObj;
    });

    const typeDefenseData = await Promise.all(typeDefensePromises);
    setTypeDefense(typeDefenseData);
  };
  ...
  if (!pokemon.id || !description.id || !typeDefense.name) {
    return (
      <div>
        <p>Loading... </p>
      </div>

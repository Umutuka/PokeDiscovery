...
const Details = () => {
  ...
  const getPokemon = async () => {
    ...
    // Fetch the type strengths and weaknesses
    const typeRelationsRes = await fetch(types[0].damage_relations.url);
    const typeRelationsData = await typeRelationsRes.json();
  
    // Combine the strengths and weaknesses of all the Pokemon's types
    const allRelations = types.reduce(
      (acc, type) => ({
        double_damage_to: [...acc.double_damage_to, ...type.damage_relations.double_damage_to],
        double_damage_from: [...acc.double_damage_from, ...type.damage_relations.double_damage_from],
        half_damage_to: [...acc.half_damage_to, ...type.damage_relations.half_damage_to],
        half_damage_from: [...acc.half_damage_from, ...type.damage_relations.half_damage_from],
        no_damage_to: [...acc.no_damage_to, ...type.damage_relations.no_damage_to],
        no_damage_from: [...acc.no_damage_from, ...type.damage_relations.no_damage_from],
      }),
      {
        double_damage_to: [],
        double_damage_from: [],
    ...
    // Set the Pokemon's strengths and weaknesses in state
    setStrengths(allRelations.double_damage_to);
    setWeaknesses(allRelations.double_damage_from);
  };

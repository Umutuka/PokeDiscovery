...
const Breeding = ({ eggGroups, gender, eggCycles }) => {
    const malePercentage = parseInt((gender / (gender + 1)) * 100).toFixed(0);
    const femalePercentage = parseInt((1 / (gender + 1)) * 100).toFixed(0);
  ...
  return (
    <div className="p-1 pb-10 space-y-2 text-base font-semibold border rounded-lg bg-slate-300 text-left">
      ...
      <p className="text-base ml-2">Gender: {malePercentage} {femalePercentage}</p>

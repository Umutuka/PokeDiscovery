...
const Breeding = ({ eggGroups, gender, eggCycles }) => {
    const femalePercentage = Math.floor((gender / (gender + 1)) * 100);
    const malePercentage = Math.ceil((1 / (gender + 1)) * 100);
  ...
  return (
    <div className="p-1 pb-10 space-y-2 text-base font-semibold border rounded-lg bg-slate-300 text-left">
      ...
      <p className="text-base ml-2">Gender: {malePercentage}% Male {femalePercentage}</p>

...
const BaseStats = ({ stats }) => {
  ...
  return (
    <div className="p-1 pb-10 max-h-96 space-y-2 text-base font-semibold border rounded-lg bg-slate-300 text-left">
      ...
      <div className="flex flex-col space-y-2">
        {stats.map(stat => (
          <div key={stat.stat.name} className="flex items-center justify-between">
            <div className="w-20">{stat.stat.name.charAt(0).toUpperCase() + stat.stat.name.slice(1)}</div>

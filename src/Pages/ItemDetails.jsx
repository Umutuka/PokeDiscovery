...
const fetchItem = async (id) => {
  const previousItemResponse =
    id - 1 > 0
      ? await fetch(`https://pokeapi.co/api/v2/item/${id - 1}/`).then((res) =>
          res.json()
        )
      : null;
  const nextItemResponse = await fetch(
    `https://pokeapi.co/api/v2/item/${Number(id) + 1}/`
  ).then((res) => res.json());
...
const ItemDetails = () => {
  ...
  const { isLoading, error, data } = useQuery(["item", id], () =>
    fetchItem(id)
  );
              ...
              >
                <p>
                  ...
                  {"<= #" +
                    (item.id - 1) +
                    " " +
                    previousItem.name.charAt(0).toUpperCase() +
                    previousItem.name.slice(1)}
                </p>
              </Link>
              ...
              >
                <p>
                  {"#" +
                    (item.id + 1) +
                    " " +
                    nextItem.name.charAt(0).toUpperCase() +
                    nextItem.name.slice(1) +
                    "=>"}
                </p>
              </Link>
            </div>
